...
public class ExecuteTests 
{
  ...
  public void login() throws Exception 
  {
	  ...
	  Sheet sh=ref.readExcel(System.getProperty("user.dir")+"\\src\\testData\\","TestCase.xlsx" , "Sheet1");
      ...
      //Create a loop over all the rows of excel file to read it
  	  for (int i = 1; i < rowCount+1; i++) 
  	  {
  		...
  		if(row.getCell(0).toString().length()==0)
  		{
  		 if(row.getCell(1).toString().length()==0)
  		 {
            ...
            try 
            {
                ...
                wef.writeExel(System.getProperty("user.dir")+"\\src\\testData\\","TestCase.xlsx" , "Sheet1",row.getRowNum(),"Pass");
            }
            catch(Exception e)
            {
            	e.printStackTrace();
            	wef.writeExel(System.getProperty("user.dir")+"\\src\\testData\\","TestCase.xlsx" , "Sheet1",row.getRowNum(),"Fail - "+e.getMessage());
  		...
  		else
  		{
  			...
  			wef.writeExel(System.getProperty("user.dir")+"\\src\\testData\\","TestCase.xlsx" , "Sheet1",row.getRowNum(),"New TC Started");
  		}
  	   }
  	   else
  	   {
 			wef.writeExel(System.getProperty("user.dir")+"\\src\\testData\\","TestCase.xlsx" , "Sheet1",row.getRowNum(),"New Module Started");			  
  	   }
  	  }
   }
